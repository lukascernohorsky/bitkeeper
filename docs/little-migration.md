...
- Iteration 19: Rescanned (`rg -n "bk little|bk L|_little|_little_gui|little_gui" . | head`) to refresh the remaining Little surface area. Outstanding conversions are the `citool.l` GUI launcher, the `src/contrib/git2bk.l` importer, and the `src/gui/tcltk/tcl/generic/Lscanner.l` grammar in the bundled Tcl tree. Gate hits are limited to documentation and the noted Little sources.
